...
//compare int to multiple
	public static String compare(String testName, int result, int[] correct){
		...
		String rep = RED+CROSS+"  "+testName+" Failed"+RESET+"\n\tExpected one of the following: [";
		for(int c: correct){
			rep += c+", ";
		}
		rep += "]\n\tBut was: "+result;
...
//compare arraylists<string>
	public static String compare(String testName, ArrayList<String> result, ArrayList<String> correct){
		if(result.equals(correct)){
			return GREEN+CHECK+"  "+testName+" Passed"+RESET;
		}
		return RED+CROSS+"  "+testName+" Failed"+RESET+"\n"+diff(result,correct);
	}
	public static String diff(ArrayList<String> result, ArrayList<String> correct){
		if (result.size() != correct.size()){
			return "\tArrayList lengths differ!\n\tExpected: "+correct.size()+" elements\n\tBut was: "+result.size()+" elements";
		}
		for(int i = 0; i < correct.size(); i++){
			if(! result.get(i).equals(correct.get(i))){
				return "\tArrayLists differ at index "+i+"\n\tExpected: "+correct.get(i)+"\n\tBut was: "+result.get(i);
			}
		}
		return "error comparing ArrayList<Object>";
	}
//compare string to string[]
	public static String compare(String testName, String result, String[] correct){
		for(int i = 0; i < correct.length; i++){
			if(result.equals(correct[i])){
				return GREEN+CHECK+"  "+testName+" Passed"+RESET;
			}
		}
		return RED+CROSS+"  "+testName+" Failed"+RESET+"\n"+diff(result, correct[0]);
	}
//compare strings
	public static String compare(String testName, String result, String correct){
		if(result.equals(correct)){
    ...
    //If there wasn't any \n add the whole thing
		if(list.size()<1){
			list.add(temp);
		}//check to see if the last line didn't have a \n and needs to be added
    else if(! list.get(list.size()-1).equals(temp)){
      list.add(temp);
    }
		return list;
	}

public static int diff_index(String rline, String cline){
  //strings rline and cline are not equal
  char rc;
  char cc;
  //find smallest line
  int small = rline.length();
  int cs = cline.length();
  if (small > cs){
    small = cs;
  }
  //search the similar lengths to see if the difference is there
  ...
  //difference wasn't in the similar length
  return -1;
}
	...
	public static int[] locate(ArrayList<String> rList, ArrayList<String> cList){
		...
		for(int line = 0; line < smallA; line++){
			...
			loc[0] = line;
			if(! rline.equals(cline)){//if the lines are not equal
        loc[1] = diff_index(rline, cline);
        if (loc[1] != -1){//we found the difference
          return loc;
				}
