...
//compare double[]
public static String compare(String testName, double[] result, double[] correct){
	if(Arrays.equals(result, correct)){
		return GREEN+CHECK+"  "+testName+" Passed"+RESET;
	}
	return RED+CROSS+"  "+testName+" Failed"+RESET+"\n"+diff(result,correct);
...
public static String diff(double[] result, double[] correct){
	if (result.length != correct.length){
		return "\tArray lengths differ!\n\tExpected: "+correct.length+" elements\n\tBut was: "+result.length+" elements";
	}
	for(int i = 0; i<correct.length;i++){
		if(Math.abs(correct[i]-result[i])>0.00001){
			return "\tArrays differ at index "+i+"\n\tExpected: "+correct[1]+"\n\tBut was: "+result[i];
		}
	}
	return "error comparing double[]";
}
//compare string[]
...
//compare int[][]
	...
	public static String diff(int[][] result, int[][] correct){
		...
		for(int i = 0; i<correct.length;i++){
			if(!Arrays.equals(correct[i],result[i])){
				for (int j = 0; j < 2; j++){
					if(correct[i][j]!= result[i][j]){
						return "\tArrays differ at index "+j+"of element "+i+"\n\tExpected: "+correct[i][j]+"\n\tBut was: "+result[i][j];
					}
				}
			}
		}
